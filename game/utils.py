...
class Text:
    def __init__(
        ...
        self.image = video.Texture.from_surface(
            renderer, render_text(text, size, color)[0]
        )
        ...
        self.rect = pygame.Rect(
            self.location[0], self.location[1], self.image.width, self.image.height
        )
        if self.centered:
    ...
    def update_text(self, newtext):
        self.image = video.Texture.from_surface(
            self.renderer, render_text(newtext, *self.settings)[0]
        )
...
class LinedText:
    def __init__(
        ...
        self.images = [
            render_text(t, size, color)[0] for t in textwrap.wrap(text, n_charwrap)
        ]
